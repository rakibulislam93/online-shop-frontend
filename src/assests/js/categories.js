...
const loadCategory =()=>{
    ...
    .then(data=>{
        ...
        data.forEach(element => {
            ...
            categoryCard.className = "border w-40 p-2 h-36 transition-all duration-300 hover:scale-105 hover:shadow-xl cursor-pointer"
...
const loadProductCategoryWise =()=>{
    ...
    const product_category = new URLSearchParams(window.location.search).get('product_category')
    const min_price = new URLSearchParams(window.location.search).get('min_price')
    const max_price = new URLSearchParams(window.location.search).get('max_price')
    ...
    .then(data=>{
        ...
        data.forEach(product => {
            ...
            catwise_productCard.className = "w-60 border transition-all duration-300 hover:scale-105 hover:shadow-xl cursor-pointer bg-white"
